...
export const SocialLayout: React.FC<Props> = ({ onBack, initialView = 'feed' }) => {
    ...
    return (
        <div className={`flex-1 w-full min-h-screen bg-black text-[#e7e9ea] font-sans flex justify-center selection:bg-[var(--accent-color)] selection:text-white`} dir="rtl">
            ...
            {!isDeepPage && (
            <div className={`hidden md:flex flex-col w-20 xl:w-72 h-screen sticky top-0 px-2 xl:px-4 border-l border-[#2f3336] justify-between py-4 z-50 bg-black`}>
                <div className="space-y-1">
                    <div className="px-3 py-3 w-fit mb-2 hover:bg-[#16181c] rounded-full cursor-pointer transition-colors" onClick={onBack}>
                        <div className="flex items-center gap-3">
                            <div className="w-8 h-8 bg-white rounded-full flex items-center justify-center">
                                 <span className="font-black text-black text-lg">M</span>
                            </div>
                            <span className="hidden xl:block text-xl font-extrabold tracking-wide text-[#e7e9ea]">مجتمع ميلاف</span>
                        </div>
                    </div>
                ...
                <div className="space-y-2">
                     ...
                     <div 
                        onClick={cycleTheme}
                        className="flex items-center gap-3 p-3 rounded-full hover:bg-[#16181c] cursor-pointer xl:w-full transition-colors group text-[#e7e9ea]"
                        title="Change Theme"
                     >
                         <ThemeIcon />
                     ...
                     {user ? (
                        <div className="flex items-center gap-3 p-3 rounded-full hover:bg-[#16181c] cursor-pointer xl:w-full transition-colors">
                            ...
                            <div className="hidden xl:block flex-1 min-w-0 text-right">
                                <div className="font-bold text-[#e7e9ea] text-sm truncate">{user.name}</div>
                                <div className="text-[#71767b] text-xs truncate">@{user.username || 'user'}</div>
                            </div>
                            <MoreHorizontal className="hidden xl:block w-5 h-5 text-[#71767b]"/>
                        </div>
                     ) : (
                        <button 
                            onClick={() => signInWithGoogle()}
                            className="flex items-center gap-3 p-3 rounded-full hover:bg-[#16181c] cursor-pointer xl:w-full transition-colors border border-[#2f3336]"
                        >
                            <LogIn className="w-6 h-6 text-[#e7e9ea]"/>
                            <div className="hidden xl:block text-right">
                                <div className="font-bold text-[#e7e9ea] text-sm">تسجيل الدخول</div>
                                <div className="text-[#71767b] text-xs">Google Account</div>
                            </div>
                        </button>
                     )}
                </div>
            ...
            {/* Main Content Area */}
            <main className={`flex-1 w-full min-h-screen pb-20 md:pb-0 relative border-r border-[#2f3336] bg-black ${isDeepPage ? 'max-w-full' : 'max-w-4xl'}`}>
                
                {/* Mobile Top Bar */}
                {(!isDeepPage && view !== 'chat' && view !== 'post-detail') && (
                    <div className="md:hidden sticky top-0 z-30 bg-black/85 backdrop-blur-md border-b border-[#2f3336] px-4 py-3 flex justify-between items-center text-[#e7e9ea]">
                ...
                {view === 'profile' && user && (
                    <div className="min-h-screen bg-black">
                        <div className="sticky top-0 z-30 bg-black/85 backdrop-blur-md border-b border-[#2f3336] px-4 py-3 flex items-center gap-6 hidden md:flex">
                            <button onClick={() => setView('feed')} className="hover:bg-[#16181c] p-2 rounded-full transition-colors text-[#e7e9ea]">
                                <ArrowLeft className="w-5 h-5 rtl:rotate-180"/>
                            </button>
                            <div>
                                <h2 className="font-bold text-lg text-[#e7e9ea]">{user.name}</h2>
                                <p className="text-xs text-[#71767b]">{user.publisherStats?.totalSales || 0} منشور</p>
                            </div>
                        </div>
                        <ProfileHeader user={user} isOwnProfile={true} />
                        <div className="p-10 text-center text-[#71767b] text-sm">
                            لا توجد منشورات بعد.
                        </div>
                    </div>
                )}
                ...
                {view === 'messages' && (
                    <div className="min-h-screen bg-black">
                        <div className="p-4 border-b border-[#2f3336] font-bold text-xl flex justify-between items-center text-[#e7e9ea]">
                            <span>الرسائل</span>
                            <Mail className="w-5 h-5"/>
                        </div>
                        {/* Mock Chat List */}
                        {[1, 2, 3].map(id => (
                            <div key={id} onClick={() => handleChatClick(id.toString())} className="flex gap-3 p-4 hover:bg-[#16181c] cursor-pointer transition-colors border-b border-[#2f3336]">
                                <img src={`https://api.dicebear.com/7.x/initials/svg?seed=${id}`} className="w-12 h-12 rounded-full border border-[#2f3336]"/>
                                <div className="flex-1">
                                    <div className="flex justify-between">
                                        <span className="font-bold text-[#e7e9ea]">مستخدم {id}</span>
                                        <span className="text-xs text-[#71767b]">10:00 AM</span>
                                    </div>
                                    <p className="text-[#71767b] text-sm truncate">مرحباً، هل الخدمة متاحة؟</p>
                                </div>
                            </div>
                        ))}
                    </div>
            ...
            {!isDeepPage && (
            <div className="hidden lg:block w-80 xl:w-96 h-screen sticky top-0 px-6 py-4 border-r border-[#2f3336] z-40 bg-black">
                <div className="relative mb-6 group">
                    <input 
                        type="text" 
                        placeholder="بحث في مراد سوشل..." 
                        className="w-full bg-[#202327] border-none focus:ring-1 focus:ring-[var(--accent-color)] rounded-full py-3 pr-12 pl-4 text-sm text-[#e7e9ea] placeholder-[#71767b] outline-none transition-all"
                    />
                    <Search className="absolute right-4 top-3 w-5 h-5 text-[#71767b]"/>
                </div>

                <div className="bg-[#16181c] rounded-2xl overflow-hidden mb-4">
                    <div className="p-4">
                        <h3 className="font-black text-lg text-[#e7e9ea]">المتداول لك</h3>
                    </div>
                    <div>
                        {[
                            ...
                            <div key={i} className="px-4 py-3 hover:bg-white/5 cursor-pointer transition-colors">
                                <div className="font-bold text-[#e7e9ea]">{trend.tag}</div>
                                <div className="text-xs text-[#71767b] mt-0.5">{trend.posts} منشور</div>
                            </div>
                        ))}
                    </div>
                </div>
            ...
            {isComposeOpen && (
                <div className="fixed inset-0 z-[100] bg-black flex flex-col p-4 animate-in slide-in-from-bottom-10 duration-200">
                    <div className="flex justify-between items-center mb-6">
                        <button onClick={() => setIsComposeOpen(false)} className="text-white font-bold">إلغاء</button>
                        <button 
                    ...
                    <div className="flex gap-3">
                        ...
                        <textarea 
                            className="flex-1 bg-transparent text-white text-lg placeholder-[#71767b] outline-none resize-none h-[40vh]"
                    ...
                    <div className="mt-auto border-t border-[#2f3336] pt-4 flex gap-6 text-[var(--accent-color)]">
...
const SidebarLink = ({ icon: Icon, label, active, onClick, notifyCount }: any) => (
    <button 
        onClick={onClick}
        className={`relative flex items-center gap-4 px-4 py-3 rounded-full text-xl transition-all w-fit xl:w-full group ${active ? 'font-bold text-[#e7e9ea]' : 'text-[#e7e9ea] hover:bg-[#16181c]'}`}
    >
        <div className="relative">
            <Icon className={`w-7 h-7 ${active ? 'fill-current' : ''}`} />
            {notifyCount > 0 && (
                <span className="absolute -top-1 -right-1 bg-[var(--accent-color)] text-white text-[10px] min-w-[18px] h-[18px] flex items-center justify-center rounded-full border-2 border-black">
...
const NavButton = ({ icon: Icon, active, onClick, notifyCount }: any) => (
    <button 
        onClick={onClick} 
        className={`p-2 rounded-xl relative transition-colors ${active ? 'text-[#e7e9ea]' : 'text-[#71767b]'}`}
    >
