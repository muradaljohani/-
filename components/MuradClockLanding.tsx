...
import { Send, Bot, User, Sparkles, Zap, Share2, Check, ArrowLeft, Clock, AlertTriangle, PlayCircle } from 'lucide-react';
...
export const MuradClockLanding: React.FC = () => {
  ...
  const [messages, setMessages] = useState<Message[]>([
    { id: '1', role: Role.MODEL, content: 'مرحباً! أنا مراد كلوك. المساعد الذكي لإدارة منظومتك. كيف يمكنني مساعدتك اليوم؟' }
  ]);
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    let fullText = '';
    // Use messages excluding the placeholder we just added
    ...
    try {
        await streamChatResponse(
            ...
            () => {}, // No sources needed for general chat usually
            abortControllerRef.current.signal
        );
    } catch (err) {
        setMessages(prev => prev.map(m => m.id === botMsgId ? { ...m, content: "عذراً، حدث خطأ في الاتصال بالنظام." } : m));
    } finally {
        setIsTyping(false);
  ...
  const handleSharePage = async () => {
    const shareUrl = window.location.origin + '/murad-clock';
    const shareData = {
      ...
      <main className="relative z-10 pt-16 pb-32 px-4">
        <div className="max-w-7xl mx-auto grid lg:grid-cols-2 gap-16 items-center">
           ...
           <div className="text-center lg:text-right space-y-8">
              ...
              <div className="relative group max-w-xl mx-auto lg:mx-0 animate-fade-in-up" style={{animationDelay: '0.2s'}}>
                {/* GLASSY EFFECT FOR DESCRIPTION */}
                <div className="absolute -inset-1 bg-gradient-to-r from-cyan-500/30 to-purple-600/30 rounded-2xl blur-lg opacity-50 group-hover:opacity-100 transition duration-500"></div>
                <div className="relative bg-white/5 backdrop-blur-xl border border-white/20 p-8 rounded-2xl shadow-2xl overflow-hidden">
                    <div className="absolute top-0 right-0 w-20 h-20 bg-cyan-500/10 rounded-full blur-xl"></div>
                    <div className="relative z-10">
                        <div className="flex items-center gap-2 mb-3">
                            <Clock className="w-5 h-5 text-cyan-400"/>
                            <span className="text-cyan-400 font-bold text-sm uppercase tracking-widest">Murad Clock Core</span>
                        </div>
                        <p className="text-lg md:text-xl text-gray-100 leading-relaxed font-light font-sans">
                            أكثر من مجرد نظام إدارة. إنه مساعدك الشخصي، مدير عملياتك، ومحرك نمو أكاديميتك. مدعوم بخوارزميات <span className="font-bold text-white">Murad Cloud</span> العصبية.
                        </p>
                    </div>
                </div>
              </div>
           ...
           <div id="chat-demo-container" className="relative animate-fade-in-up" style={{animationDelay: '0.4s'}}>
              ...
              <div className="relative bg-[#0b1120]/80 backdrop-blur-xl border border-white/10 rounded-3xl shadow-2xl overflow-hidden flex flex-col h-[500px]">
                 ...
                 <div className="flex-1 overflow-y-auto p-4 space-y-4 scrollbar-thin scrollbar-thumb-white/10 scrollbar-track-transparent">
                    {messages.map((msg) => (
                       <div key={msg.id} className={`flex items-end gap-3 ${msg.role === Role.USER ? 'flex-row-reverse' : 'flex-row'}`}>
                          ...
                          <div className={`max-w-[80%] p-3 rounded-2xl text-sm leading-relaxed ${
                             ...
                             {msg.content}
                             {msg.isStreaming && <span className="inline-block w-1.5 h-4 bg-cyan-400 ml-1 animate-pulse align-middle"></span>}
                          </div>
                       </div>
                    ))}
