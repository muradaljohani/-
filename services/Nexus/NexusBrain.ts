...
export class NexusBrain {
    ...
    public async globalQuery(term: string): Promise<SearchResult[]> {
        ...
        // 1. Query Jobs DB
        const jobs = JSON.parse(localStorage.getItem('allJobs') || '[]') as UserJob[];
        jobs.forEach(j => {
        ...
        // 2. Query Courses (LMS)
        const storedCourses = JSON.parse(localStorage.getItem('mylaf_custom_courses') || '[]');
        ...
        const products = JSON.parse(localStorage.getItem('allProducts') || '[]') as ProductListing[];
        const services = JSON.parse(localStorage.getItem('mylaf_services') || '[]') as ServiceListing[];
