...
export const MobileSidebar: React.FC<Props> = ({ isOpen, onClose, unreadCount = 0, onNavigate }) => {
  ...
  return (
    <>
      ...
      {/* Sidebar Panel */}
      <div 
        className={`fixed inset-y-0 right-0 w-[80%] sm:w-[320px] bg-white dark:bg-black border-l border-gray-200 dark:border-[#2f3336] z-[60] shadow-2xl transform transition-transform duration-300 ease-out flex flex-col ${
      ...
      >
        <div className="p-5">
          ...
          {user && (
            <div className="flex gap-4 text-sm mb-4">
                ...
                <span className="font-bold text-black dark:text-[#e7e9ea]">{user.following?.length || 142}</span>
                <span className="text-gray-500 dark:text-[#71767b]">متابعة</span>
                </div>
                <div className="flex gap-1">
                <span className="font-bold text-black dark:text-[#e7e9ea]">{user.followers?.length || 11711}</span>
        ...
        <div className="border-t border-gray-200 dark:border-[#2f3336] p-4 flex justify-between items-center bg-white dark:bg-black">
            {user ? (
                ...
                <button onClick={() => { /* Should trigger login flow in layout */ onClose(); }} className="flex items-center gap-2 text-blue-500 font-bold text-sm">
                    تسجيل الدخول
                </button>
            )}
